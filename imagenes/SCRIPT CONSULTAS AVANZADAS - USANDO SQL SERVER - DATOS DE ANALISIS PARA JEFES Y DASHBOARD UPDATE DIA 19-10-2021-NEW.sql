...
use  EJEMPLO_DCD_1050_AGOSTO_DICIEMBRE_2022_V2;
go
...
ALTER AUTHORIZATION ON DATABASE::EJEMPLO_DCD_1050_ENERO_ABRIL_2022_V1 TO JUANCITO
...
select * from departamento where nombre_departamento='tecnologia'

--MOSTRARME EL TOTAL DE DEPARTAMENTOS

select count(*) as [Total Departamentos] from departamento
...
datediff(MONTH, max(fecha_venta), '23/11/2022') as UltimaFecha,
NTILE(5) over (order by datediff(MONTH, max(fecha_venta), '31/12/2021') desc) as Calificacion_Fresencia,
